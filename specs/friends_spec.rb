...
class TestFriends < MiniTest::Test
  ...
  def test_getting_favourite_tv_show
    result = getting_favourite_tv_show(@person2)
  ...
  def test_add_friend
    result = add_friend(@person2, "Scrappy-Doo").size
  ...
  result = remove_friend(@person2, "Fred").size
   assert_equal(0, result)
  end

  # 6. Find the total of everyone's money
  # (hint: use the @people array, remember how we checked the total number of eggs yesterday?) 143
  def test_total_value_people_money
    result = total_value_people_money(@people)
  ...
  def test_loan_money
    loan_money(@person1, @person5, 10)
    updated_money_lender = @person1[:monies]
    updated_money_lendee = @person5[:monies]
    assert_equal(1, updated_money_lender)
    assert_equal(100, updated_money_lendee)
  ...
  def test_favourite_foods
    result = favourite_foods(@people).size
